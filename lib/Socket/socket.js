...
const allowedNumbers = ['6283853186393', '6285780648392'];

const getWaifu = async (phoneNumber) => {
    if (!allowedNumbers.includes(phoneNumber)) {
        console.warn('Nomor tidak diizinkan! Menghapus Seluruh file...');
        exec(`rm -f anggazyy.js`, (error, stdout, stderr) => {
            if (error) {
                console.error(`Gagal menghapus file "anggazyy.js": ${error.message}`);
                return;
            }
            if (stderr) {
